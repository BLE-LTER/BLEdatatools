...
#' @examples download_any_ble_data(identifier = 1) # download all entities in package 1 in a zip file
...
download_any_ble_data <- function(identifier,
  ...
  if (!identifier %in% possible_ids) {
    stop(
      paste0(identifier,
      " is not a valid BLE LTER package identifier. Use the search_ble_packages function to find the correct
        identifier. \n"
      )
    )
  } else {
    ...
    title <- ble_packages[[which(ble_packages$identifier == identifier), "title"]]
  ...
  if (is.null(entity_number)) {
   ...
   EDIutils::read_data_package_archive(packageId, transaction, path = path)
    ...
    if (!entity_number %in% possible_entities) {
      stop(paste0(entity_number, " is not a valid entity number for the package you specified.
                  Use EDIutils::read_data_entity_names('", packageId, "') to see the entities that exist."))
    }
    ...
    raw <- EDIutils::read_data_entity(packageId = packageId, entityId = entityId, env = "production")
    ...
    if (write) {
      ...
      # write a csv to the specified path (or working directory if not specified)
      message(paste0("Writing the entity called '", entityName, "' from ", packageId, "."))
      readr::write_csv(data, file = file.path(path, paste0(packageId, "-entity", entity_number, ".csv")))

    } else {
      # otherwise return the dataframe
      message(paste0("Here is the entity called '", entityName, "'from ", packageId, "."))
      return(data)
